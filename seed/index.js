...
const generatePictures = (numberOfPictures, numberOfUsers) => {
  ...
  for (let i = 0; i < numberOfPictures; ++i) {
    pictures.push({
      ...
      S3URL: `http://placekitten.com/${Math.ceil(
        Math.random() * 200 + 200
      )}/${Math.ceil(Math.random() * 200 + 200)}`,
    });
  }
  return pictures;
};
...
const seed = async () => {
  try {
    ...
    const NUMBER_OF_PICTURES = 100;
    const NUMBER_OF_TAGS = 90;
    const NUMBER_OF_COMMENTS = 500;
    const NUMBER_OF_GALLERIES = 50;
    ...
    await Picture.bulkCreate(
      generatePictures(NUMBER_OF_PICTURES, NUMBER_OF_USERS)
    );
    await Tag.bulkCreate(generateTags(NUMBER_OF_TAGS));
    await PictureTag.bulkCreate(
      generatePictureTags(NUMBER_OF_PICTURES, NUMBER_OF_TAGS)
    );
    //Create Comments
    await Comment.bulkCreate(
      generateComments(NUMBER_OF_COMMENTS, NUMBER_OF_PICTURES, NUMBER_OF_USERS)
    );

    //Create Galleries
    await Gallery.bulkCreate(
      generateGalleries(NUMBER_OF_GALLERIES, NUMBER_OF_USERS)
    );
    await GalleryPicture.bulkCreate(
      generateGalleryPictures(
        NUMBER_OF_GALLERIES,
        NUMBER_OF_PICTURES,
        NUMBER_OF_USERS
      ),
    ...
    await GalleryUser.bulkCreate(
      generateGalleryUsers(NUMBER_OF_GALLERIES, NUMBER_OF_USERS)
    );
